...
int32_t rtc_open(const uint8_t* filename){
    ...
    rtc_info[idx].rtc_syshz_per_uhz = RTC_USR_DEFAULT; //since its init, the system frequency of interrupts will always be 1 of itself
...
int32_t rtc_close(int32_t fd){
    ...
    uint32_t pid = pcb_address->pid;
    uint32_t idx = get_process_terminal(pid);
    rtc_info[idx].rtc_ctr = 1;
    rtc_info[idx].rtc_syshz_per_uhz = RTC_USR_DEFAULT; //reset the # of interrupts per system freq to be just 1, so its itself
    return RTC_PASS;
}
